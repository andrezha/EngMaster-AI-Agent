...
class VocabManager(QObject): # 继承自 QObject
    ...
    mistake_vocabulary_changed = Signal() # Define the signal
    ...
    def __init__(self, main_window_instance):
        ...
        if not self.vocab_page_widget:
            raise AttributeError("Vocabulary page widget not found in stackedWidget at index 0.")

        # 查找控件
        self.v_input = self.vocab_page_widget.findChild(QtWidgets.QLineEdit, "vocab_input")
        self.v_disp = self.vocab_page_widget.findChild(QtWidgets.QTextEdit, "vocab_display")
        self.t_label = self.vocab_page_widget.findChild(QtWidgets.QLabel, "timer_label")
        self.btn_confirm = self.vocab_page_widget.findChild(QtWidgets.QPushButton, "btn_confirm")
        
        # 新增：闯关模式选择按钮和打印控件
        self.btn_challenge_regular = self.vocab_page_widget.findChild(QtWidgets.QPushButton, "btn_challenge_regular") # 查找现有按钮
        ...
        # 初始化计时器
        self.timer = QtCore.QTimer() # Keep timer initialized to prevent AttributeError on .stop()
    ...
    def _rebuild_layout(self):
        ...
        while self.main_layout.count():
            ...
            if item.widget():
                ...
                else: item.widget().deleteLater() # Delete other widgets
            elif item.layout(): self._clear_layout(item.layout(), widgets_to_keep) # Recursively clear nested layouts, passing widgets_to_keep
        ...
        self.main_layout.addWidget(self.v_input, alignment=QtCore.Qt.AlignmentFlag.AlignCenter)
        ...
        self.main_layout.addWidget(self.btn_confirm, alignment=QtCore.Qt.AlignmentFlag.AlignCenter)
        
        # 8. 底部弹簧 - 黄金重心（2倍，使内容上移）
        self.main_layout.addStretch(2)

    def _clear_layout(self, layout, widgets_to_keep_set): # Accept widgets_to_keep_set as an argument
        """
        ...
        while layout.count():
            ...
            elif item.layout(): self._clear_layout(item.layout(), widgets_to_keep_set) # Recursive call with the argument
    ...
    def _style_components(self):
        ...
        if self.v_disp: # 确保显示区域宽度和居中由 UI 文件控制
            self.v_disp.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter) # 保持文本居中
    ...
    def _load_regular_vocabulary(self):
        ...
        try: # Access base_path from the main_window instance
            ...
            print(f"DEBUG: Attempting to load regular vocabulary from: {vocab_path}")
            ...
            print(f"❌ 加载常规词汇表失败: {e}")
            self.vocabulary = [{"word": "apple", "content": "苹果", "pronunciation": "/ˈæpl/", "example": "An apple a day keeps the doctor away."}] # Fallback with more details
            QMessageBox.warning(self.main_window, "错误", f"加载常规词汇表失败: {e}\n请检查 assets/vocabulary.json 文件。已加载默认词汇。")
    ...
    def _load_mistake_vocabulary(self):
        ...
        try:
            ...
            self.mistake_vocabulary = []
            self._add_test_mistake_words()
            QMessageBox.warning(self.main_window, "错误", f"错词表文件 {self.mistake_word_file_path} 格式错误，已重置。")
    ...
    def _save_mistake_vocabulary(self):
        ...
        try:
            with open(self.mistake_word_file_path, "w", encoding="utf-8") as f:
                json.dump(self.mistake_vocabulary, f, ensure_ascii=False, indent=4)
            print(f"✅ 错词表已保存，当前 {len(self.mistake_vocabulary)} 词。")
            self.mistake_vocabulary_changed.emit() # Emit the signal after saving
        except Exception as e: # Use self.main_window for QMessageBox
            print(f"❌ 保存错词表失败: {e}")
            QMessageBox.critical(self.main_window, "错误", f"保存错词表失败: {e}")
    ...
    def _increment_mistake_word_correct_count(self, word_obj):
        ...
        for i, item in enumerate(self.mistake_vocabulary):
            if item["word"].lower() == word_obj["word"].lower():
                ...
                print(f"⬆️ 错词 '{word_obj['word']}' 正确计数: {self.mistake_vocabulary[i]['correct_count']}")
    ...
    def load_active_vocabulary(self):
        ...
        if self.current_challenge_mode == "regular":
            ...
            if not self.active_vocabulary: # 如果错词表为空，自动切换回常规模式
                QMessageBox.information(self.main_window, "提示", "错词表为空，已自动切换到常规闯关模式。", parent=self.main_window)
    ...
    def show_next(self, error_msg="", additional_message_html=""):
        ...
        if not self.active_vocabulary:
            self.v_disp.setHtml("<div style='text-align: center; padding: 20px; font-size: 20px; color: #e74c3c;'>词汇表为空，请检查！</div>")
            self.v_input.clear()
            self.v_input.setEnabled(False)
            self.btn_confirm.setEnabled(False)
        ...
        if not (0 <= self.current_idx < len(self.active_vocabulary)):
            print(f"ERROR: current_idx ({self.current_idx}) out of bounds for active_vocabulary (len {len(self.active_vocabulary)}). Resetting to 0.")
        ...
        print(f"DEBUG: Displaying word: {curr.get('word', 'N/A')}, content: {curr.get('content', 'N/A')}")
        ...
        if curr.get('pronunciation'):
            html_content_parts.append(f"<div style='font-size: 18px; color: #666666; margin-top: 8px;'>{curr.get('pronunciation', '')}</div>")
        if curr.get('example'):
            html_content_parts.append(f"<div style='font-size: 14px; color: #888888; margin-top: 12px; font-style: italic;'>例句: {curr.get('example', '')}</div>")
        ...
        if not error_msg: # Only clear input and start timer if it's a new word (not an error display)
            self.v_input.clear()
    ...
    def tick(self):
        ...
        if self.time_left <= 0:
            ...
            if self.active_vocabulary and 0 <= self.current_idx < len(self.active_vocabulary):
                ...
                self.show_next(error_msg=target, additional_message_html=additional_message_html) # 错误时显示正确答案
                ...
                print("ERROR: Timer ticked but active_vocabulary is empty or current_idx is out of bounds.")
    ...
    def check_answer(self):
        ...
        if not self.active_vocabulary or not (0 <= self.current_idx < len(self.active_vocabulary)):
            print("ERROR: No active vocabulary or invalid current_idx when checking answer.")
            return # Do nothing if no active vocabulary or invalid index
        
        current_word_obj = self.active_vocabulary[self.current_idx]
        target = current_word_obj['word'].strip().lower()
        
        self._notification_message_for_next_display = "" # Clear any previous notification
        ...
        if user_in == target:
            ...
            if self.current_challenge_mode == "mistake_list":
                removed_from_mistake_list = self._increment_mistake_word_correct_count(current_word_obj)
